...
GroovyRowResult result =  this.sql.firstRow("select ${tableName}.userId,<#list nonRelatedFields as noRelatedField >${tableName}.${noRelatedField.name}<#if noRelatedField?has_next >,</#if></#list><#if (relatedFields?size>0)>,</#if><#list relatedFields as relatedField>${relatedField.tableName}.${relatedField.field_name}<#if relatedField?has_next>,</#if></#list> from ${tableName}  <#list relatedFields as relatedField> join ${relatedField.tableName} <#if relatedField?has_next> join </#if></#list> <#if (relatedFields?size>0)> on </#if> <#list relatedFields as relatedField>${relatedField.tableName}.id=${tableName}.${relatedField.name}</#list> where ${tableName}.deleted=false and ${tableName}.id='"+id + "' and ${tableName}.userId='"+userId+"'");
return result;
}
@Override
